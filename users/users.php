...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    $id = $_GET['id'];
    if ($db->hapus_users($id)) {
        $success_message = "Data user berhasil dihapus!";
    } else {
        $error_message = "Gagal menghapus data user!";
    }
}
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
  $keyword = $_GET['search'];
  $data_users = $db->cari_data_users($keyword);
} else {
  $data_users = $db->tampil_data_users();
}

// Get all religion data
$users = $db->tampil_users();
?>
...
<head>
    ...
    <meta name="title" content="Data Users" />
    ...
    <!-- Custom CSS for fixed header and scrollable body -->
    <style>
        .table-fixed-header {
            width: 100%;
            table-layout: fixed;
